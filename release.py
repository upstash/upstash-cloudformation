...
def check_subversion_existence(s3_client, bucket_name, module_name, major_version, subversion):
...
def upload_resource_package(relative_path_to_resource_directory, s3_client, bucket_name, module_name, major_version, subversion):
    full_version_prefix = '{}/{}/{}'.format(
        module_name, major_version, subversion)
...
def resolve_resource_packages_for_major_version(s3_client, bucket_name, module_name, major_version, subversion):
    # for every directory in the cfn-version/<<major_version>> directory
    # each directory represents a resource
    for resource_dir_name in listdir(path.join('cfn-resources', module_name, major_version)):
        full_resource_dir = path.join(
            'cfn-resources', module_name, major_version, resource_dir_name)
        print('building resource package in directory: {}'.format(full_resource_dir))
        build_resource_package(
            relative_path_to_resource_directory=full_resource_dir, subversion=subversion)
        print(
            'uploading package of directory: {}'.format(full_resource_dir))
        upload_resource_package(relative_path_to_resource_directory=full_resource_dir,
                                s3_client=s3_client, bucket_name=bucket_name, module_name=module_name, major_version=major_version, subversion=subversion)
...
if args['module_name'] not in ['upstashRedis', 'upstashKafka']:
    raise Exception('Unknown module name: {}'.format(args['module_name']))

subversion_exists = check_subversion_existence(
    s3_client=s3, bucket_name=args['bucket_name'], module_name=args['module_name'], major_version=args['major_version'], subversion=args['subversion'])
...
if subversion_exists and args['bucket_name'] == PRODUCTION_BUCKET_NAME:
    raise Exception('{}/{}/{} already exists in production bucket. You CANNOT override version in production bucket'.format(
        args['module_name'], args['major_version'], args['subversion']))
